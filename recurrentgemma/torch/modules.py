...
def _compute_cache_mask(
    ...
    q_positions = (
        torch.arange(seq_len, device=device) + cache_num_tokens[:, None]
    )
...
class LocalAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        Returns:
          ...
          then the returned updated cache is empty initialized and filled in from
          the input sequence.
        """
        b, t, _ = x.shape
        ...
        self.attn_pattern_recorder(probs)
...
class Embedder(nn.Module):
    ...
    def encode(self, x: at.Tokens) -> at.Activations:
        ...
        if self.scale_by_sqrt_dim:
            ...
            x = x * torch.tensor(math.sqrt(self.embed_dim)).type(
                torch.bfloat16
            )
        return x
