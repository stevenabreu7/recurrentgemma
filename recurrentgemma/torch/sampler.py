...
class Sampler:
    ...
    def __init__(
        ...
        """Initialize a sampler for a Griffin model.
    ...
    def _sample_step(
        ...
        end_sampling_at_eos_token: bool = False,
    ) -> SamplingState:
    ...
    def _sample_fn(
        ...
        while (sampler_state.step < sampler_state.total_steps - 1) & torch.any(
            torch.logical_not(sampler_state.done)
        ):
            sampler_state = self._sample_step(sampler_state, end_sampling_at_eos_token)
    ...
    def _get_padded_tokens(
        ...
        padded_tokens = [
            torch.concatenate([pad, input_ids], dim=0) for input_ids, pad in zip(tokens, pad_values)
        ]
    ...
    def __call__(
        ...
        """Sample a completion of the input string.
        ...
        # Text decoding.
        tokens = sampling_state.tokens_buffer

        if return_logits:
            logits = [logits[l:] for logits, l in zip(sampling_state.logits_buffer, pad_lengths)]
        else:
            logits = []
